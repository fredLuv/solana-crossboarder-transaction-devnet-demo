...
function loadStoredDemoWallet(): StoredDemoWallet | null {
  try {
...
function saveStoredDemoWallet(wallet: StoredDemoWallet | null) {
  if (!wallet) {
...
export default function App() {
  ...
  const demoWallet = useMemo(
    () => (storedDemoWallet ? Keypair.fromSecretKey(Uint8Array.from(storedDemoWallet.secretKey)) : null),
    [storedDemoWallet]
  );
  ...
  useEffect(() => {
    async function refreshDemoWalletBalance() {
      ...
      try {
        const lamports = await demoConnection.getBalance(demoWallet.publicKey, "confirmed");
        const mintAddress = devnetStatus?.stablecoinMint ? new PublicKey(devnetStatus.stablecoinMint) : null;
        const tokenBalance = mintAddress
          ? await getAccount(
              demoConnection,
              getAssociatedTokenAddressSync(mintAddress, demoWallet.publicKey),
              "confirmed",
              TOKEN_PROGRAM_ID
            ).then((account) => Number(account.amount) / 10 ** stablecoinDecimals)
              .catch(() => 0)
          : 0;
        setDemoWalletBalanceSol(Number((lamports / LAMPORTS_PER_SOL).toFixed(4)));
        setDemoWalletBalanceToken(Number(tokenBalance.toFixed(2)));
      } catch (_error) {
        setDemoWalletBalanceSol(null);
        setDemoWalletBalanceToken(null);
      }
